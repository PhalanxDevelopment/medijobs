...
{
  wp_enqueue_style('blankslate-style', get_stylesheet_uri(), array(), '6.12.2', 'all');
...
$jobInfo = [];
function get_job_information($categoryID, $subcategoryID, $returnType, $id)
{
  $http = _wp_http_get_object();
  ...
  if ($returnType === 'cities') {
    foreach ($jobInfo['cities'] as $cityJob) {
      ...
      $output .= " <div class=\"price_box__label\">" . get_field('table_city_text', $id) . "</div>";
      $output .= "<div class=\"price_box__value\">" . $cityJob['city'] . "</div>";
      $output .= "</div>" . "\n";
      $output .= "<div class='price_box price_box--amount'>";
      $output .= " <div class=\"price_box__label\">" . get_field('table_amount_text', $id) . "</div>";
      $output .= "<div class=\"price_box__value\">" . $cityJob['avg_salary'] . " RON</div>";
      $output .= "</div>" . "\n";
      $output .= "<div class='price_box'>";
      $output .= " <div class=\"price_box__label\">" . get_field('table_number_text', $id) . "</div>";
...
{
  global $post;
  return $content . ' ' . get_post_meta($post->ID, 'job_introducere', true);
}
