...
function blankslate_load_scripts() {
    wp_enqueue_style( 'blankslate-style', get_stylesheet_uri(),  array(), '6.9.10', 'all'  );
...
function my_custom_popular_posts_html_list( $popular_posts, $instance ){
    ...
    foreach( $popular_posts as $popular_post ) {
        ...
        if ( $instance['stats_tag']['author'] ) {
            ...
            $stats[] = '<div class="mj-article__author"> <span>WRITTEN BY </span>'  . sprintf( __( '%s', 'wordpress-popular-posts' ), $display_name ). sprintf( __( '%s', 'wordpress-popular-posts' ), $display_image ). '</div>';
        }
        ...
        $output .= "<a href=\"" . get_permalink( $popular_post->id ) . "\" class=\"mj-article__link\">READ ON</a>";
        $output .= $stats;
        $output .= "</div>" . "\n";
    }
...
function filter_the_content_in_the_main_loop( $content ) {
    ...
    if ( is_single() ) {
        ...
        $banner = '<div class="article_register_call_to_action"><div class="mj-grid"><div class="grid__item width-3/24 hide-on-mobile"></div><div class="grid__item width-12/24"><h3>Conectam profilul tau cu companiile care isi doresc sa te angajeze.</h3></div> <div class="grid__item width-1/24 hide-on-mobile"></div><div class="grid__item width-7/24"><a href="https://app.medijobs.co/register/" target="_blank" class="mj-btn mj-btn--secondary mj-btn--alt">Creează-ți un cont gratuit</a> </div> <div class="grid__item width-2/24 hide-on-mobile"></div></div> </div>';
